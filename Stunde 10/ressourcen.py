...
GEBAEUDE_WIRTSCHAFT = [
    ...
    {
        "baukosten":  {},                      # Kostenlos!
        "produktion": {"energie": 0.1, "gold": 0.1, "holz": 0.1},                      # Produziert nichts
    ...
    # ── Index 8: Mine ─────────────────────────────────────────────────────
    {
        "baukosten":  {"gold": 35, "energie": 10, "stein": 15},
        "produktion": {"kohle": 2},
        "verbrauch":  {},
        "max_anzahl": None,
        "freischaltung": {"typ": "forschung", "technologie": "minenbau"},
    },
    # ── Index 9: Strasse ──────────────────────────────────────────────────
    {
        "baukosten":  {"stein": 2},
...
def ressourcen_produzieren(ressourcen_dict, liste_gebaeude):
    ...
    for gebaeude in liste_gebaeude:
        ...
        verbrauch = dict(wirtschaft["verbrauch"])
        produktion = dict(wirtschaft["produktion"])
        if typ_index == 1 and forschung.ist_technologie_erforscht("reaktor_upgrade"):
            verbrauch["kohle"] = 1
            produktion["energie"] = produktion.get("energie", 0) + 3
        if typ_index == 7:
            studenten_bonus = ressourcen_dict.get("bevoelkerung", 0) // 10
            produktion["forschung"] = produktion.get("forschung", 0) + studenten_bonus
        ...
        if kann_produzieren:
            ...
            if forschung.ist_technologie_erforscht("produktion"):
                ...
                boost = 1.0
            if typ_index == 8 and _wirtschafts_tick % 10 == 0:
                produktion["eisen"] = 1
